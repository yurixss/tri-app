...
const RUN_ZONE_PACE_FACTORS: Record<number, { factor: number; name: string }> = {
  ...
  2: { factor: 1.2, name: 'Endurance' }, // ~20% mais lento
  3: { factor: 1.1, name: 'Marathon Pace' }, // ~10% mais lento
  4: { factor: 1.0, name: 'Threshold' }, // Pace de threshold
  ...
  if (data.waterType === 'openWater') {
    ...
    if (data.openWaterType === 'sea') {
      ...
      if (data.swellLevel && data.swellLevel !== 'none') {
        ...
        factors.push(
          `+${Math.round((swellFactor - 1) * 100)}% marola ${data.swellLevel === 'light' ? 'leve' : 'moderada'}`,
        );
      }
    }
...
export function calculateBikeTime(data: BikeData): ModalityResult {
  ...
  try {
    ...
    factors.push(
      `${data.ftpPercentage}% do FTP (${Math.round((data.ftp * data.ftpPercentage) / 100)}W)`,
    );
...
export function calculateTriathlonPrediction(data: TriathlonWizardData): TriathlonPrediction {
  ...
  const totalTimeSeconds =
    swim.timeSeconds + t1Seconds + bike.timeSeconds + t2Seconds + run.timeSeconds;
